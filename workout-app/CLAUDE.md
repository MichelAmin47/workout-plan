...
**Prefill.** `exercises.reps` (a `text` column, previously always `NULL`) was backfilled once via a regex over `exercises.sets` (`^\d+x(\d+)(/\w+)?$`, matching `"3x8"` and `"3x12/been"` alike — the per-limb number is used as-is, since it already means "reps per side"). This covers ~74% of rows; HIIT (`"30s/20s"`), time-hold (`"2x20sec/zij"`), and distance (`"3x20m"`) rows are left `NULL` — genuinely not a rep count. `buildWeeks`'s `toEx()` carries this through as `ex.reps` (`Number` or `null`). `prescribedReps`, a plain `{ [wKey]: reps }` lookup built once per render in `FitnessSchema` (after the `schemaLoading` guard — not memoized, same as `CORE_EXERCISES` above it), is consulted by `effectiveReps(exercise, weekNum, person)`: it returns the actually-logged value if one exists, otherwise the prescribed number, otherwise `""`.
