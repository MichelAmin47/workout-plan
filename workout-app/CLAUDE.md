...
- **`FitnessSchema`** (default export) — top-level component. Manages state: `selectedWeek`, `selectedDay` (index 0–6 into `week.days`, persisted to `localStorage["selectedDay"]`), `schema` (object `{ weeks, restDayGoals }` — the full built schema plus rest-day goal rows from Supabase, cached in `localStorage["cached_schema_v5"]`), `schemaGen` (integer counter incremented on every successful schema refresh — used as part of `WeekDayTile` keys to force remount after pull-to-refresh), `weights`, `expandedExercise`, `completedDays`, `completedExercises`, `swaps`, `swapModal`, `activeTimer`, `activeSection`, `timerLocked`, `schemaOffline` (bool — true when Supabase unreachable and serving from cache), `refreshing` (bool — pull-to-refresh in progress), `pullY` (raw px pulled). Also: `progressieOpen`, `progressieExercise`. Calls `useTimer`. The app header shows the displayed schema's week range ("Week 37 – 43", from `week.startWeek`/`week.eindWeek`, set in `buildWeeks` from `schemas.start_week`/`eind_week`) plus the phase badge underneath — no separate eyebrow line (removed; used to hardcode "Basic Fit · Gevorderd" above a hardcoded "7-Weken Trainingsschema" title, wrong for two of the three schemas once the app became a continuous multi-block schedule). A stale `cached_schema_v5` blob predating this field falls back to a plain "Trainingsschema" (no count) until the next background refetch fills it in — never the old hardcoded count, which is exactly what this replaced. Offline shows a gray banner below the header.
