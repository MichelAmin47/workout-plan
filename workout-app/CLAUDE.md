...
Alongside weight, one rep count per exercise per week per person is logged on `weights.reps` (nullable, no CHECK — matches `weight`'s own lack of validation). It rides the exact same write path as weight: `saveMeasurement(exercise, weekNum, person, weightValue, repsValue)` (renamed from `saveWeight`) upserts both columns in one call, still gated on **weight** being non-empty — reps alone never triggers a write, so an exercise logged before this feature shipped (or a bodyweight/core exercise, which has no weight field to gate on at all) simply has `reps = NULL`, the normal case, not an edge case.
