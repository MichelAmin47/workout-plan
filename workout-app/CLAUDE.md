...
- **`schema`** — dynamic object built from Supabase (`schemas`, `schema_days`, `exercises`, `schema_weeks` tables). Cached in `localStorage["cached_schema_v5"]` for offline use. Phase label per week comes from `schema_weeks` (values seen so far: `"Deload"`, `"Opbouw"`, `"Nieuwe Prikkel"`); a week with no `schema_weeks` row falls back to the old rule `relWeek <= 3 ? "Opbouw" : "Nieuwe Prikkel"`. Built by `buildWeeks()` at module scope.
- **`buildWeeks(schemas, schemaDays, exercises, weekOverrides = [], schemaWeeks = [])`** — module-scope pure function. Transforms raw Supabase rows into week/day structures. Iterates all schemas in one pass, producing a flat array of all calendar weeks across both schemas. Each week object includes `{ week, label, phase, schemaId, days }`. `phase` is looked up in `schemaWeeks` by `(schema_id, week_nummer)` with the hardcoded rule as per-week fallback. Each week has 7 days sorted by `dag_volgorde`. Training days have `barbell/spiergroep/kettlebell/core`; non-training days have only metadata (`type`, `dag_label`, `emoji`, `naam`). `schemaId` is used downstream to filter `restDayGoals` per schema.
- **`FitnessSchema`** (default export) — top-level component. Manages state: `selectedWeek`, `selectedDay` (index 0–6 into `week.days`, persisted to `localStorage["selectedDay"]`), `schema` (object `{ weeks, restDayGoals }` — the full built schema plus rest-day goal rows from Supabase, cached in `localStorage["cached_schema_v5"]`), `schemaGen` (integer counter incremented on every successful schema refresh — used as part of `WeekDayTile` keys to force remount after pull-to-refresh), `weights`, `expandedExercise`, `completedDays`, `completedExercises`, `swaps`, `swapModal`, `activeTimer`, `activeSection`, `timerLocked`, `schemaOffline` (bool — true when Supabase unreachable and serving from cache), `refreshing` (bool — pull-to-refresh in progress), `pullY` (raw px pulled). Also: `progressieOpen`, `progressieExercise`. Calls `useTimer`. The app header (top padding `14px`, tightened after the eyebrow line was briefly removed and left the header oversized) shows, top to bottom: an eyebrow reading the displayed schema's week range ("WEEK 37 – 43", from `week.startWeek`/`week.eindWeek`, set in `buildWeeks` from `schemas.start_week`/`eind_week`, white at 75% opacity — not the old eyebrow's `#888`, which was too low-contrast against orange to read mid-workout), the position within that block as the large title ("Week 2 van 7" — `week.week - week.startWeek + 1` of `week.eindWeek - week.startWeek + 1`, normal casing, no `textTransform`), and the phase badge. Title and eyebrow swapped which value each carries once (the range was briefly the title and the position the eyebrow — reversed back, since the range is fixed for the whole block while the position changes every week and is what's actually read at a glance). Both replaced a hardcoded "Basic Fit · Gevorderd" / "7-Weken Trainingsschema" pair, wrong (the count, not the eyebrow) for two of the three schemas once the app became a continuous multi-block schedule. A stale `cached_schema_v5` blob predating `startWeek`/`eindWeek` renders no eyebrow at all and falls back to a plain "Trainingsschema" title (no count) until the next background refetch fills it in — never the old hardcoded count. Offline shows a gray banner below the header.
...
- **`PowerHourCard`** — renders for `day.type === "power_hour"` days. Hardcoded layout (no Supabase fetch): "Power Hour" header, "Externe sportzaal — buiten dit trainingsschema." body, "🕐 HIIT" badge.
- **`BoksenCard`** — renders for `day.type === "boksen"` days. Same hardcoded-layout shape as `PowerHourCard`: "Boksen" header, "Boksen in Beverwijk" body, "🥊 Boksen & Zaktraining" badge. Boksen has no default slot in any schema — it only ever appears via a `week_overrides` row (see that table's schema below), never from `schema_days` directly.
  - Known inconsistency, deliberately left alone: both cards' header text uses `#ea580c`, not the `#f97316` used everywhere else for these two day types (tile accent, name-label color). Carried over unchanged from the original single cardio-fitness card; not worth unifying on its own.
...
- **`ExRow`** — single exercise row (number badge, name, optional note, sets pill). When `onToggle` is provided (spiergroep, kettlebell), the row is clickable and renders a weight input panel below it when `expanded` is true. The panel contains M: and Z: number inputs that save to Supabase on change, a reps stepper (± buttons, non-input display) next to each, plus a previous-week reference line reading e.g. "Laatste keer — Week 33 · M: 40kg × 8". The number badge is rendered by `ExCircle`. Accepts `swapped` (bool) and `originalName` (string) props — when `swapped` is true, shows a purple "GEWIJZIGD" badge next to the name and renders `↩ originalName` below in gray. Accepts `hiitInterval` (`{ work, rest }` or null) — when set, renders a split badge (`Xs | Ys`) instead of the regular sets pill. See "Reps logging" below — the reps stepper markup is duplicated (not shared) across `ExRow`, the standalone Barbell block, and `SupersetBlock`.
...
- **`dayColors` / `phaseColors`** — lookup maps from day ID / phase name to color tokens. These drive all theming; there is no CSS file. `phaseColors` has `Deload` / `Opbouw` / `Nieuwe Prikkel`; `phaseColorFallback` (neutral gray) is used for any other phase value so the header badge never crashes on an unknown label.
...
- `type` — `"training"` | `"rust"` | `"power_hour"` | `"boksen"`
...
- `phase` — `"Deload"` | `"Opbouw"` | `"Nieuwe Prikkel"` (or a future block's own label). Read from the `schema_weeks` table by `(schema_id, week_nummer)`; when a week has no row, `buildWeeks` falls back to `relWeek <= 3 ? "Opbouw" : "Nieuwe Prikkel"`.

**Supabase `schema_weeks` columns**: `schema_id` UUID (FK → `schemas.id`, ON DELETE CASCADE), `week_nummer` INT (relative week 1..N, same meaning as `exercises.week_nummer` — **not** a calendar week), `phase` TEXT NOT NULL. PK `(schema_id, week_nummer)`. No CHECK on `phase` (a new block can introduce a new label with a plain INSERT). One row per relative week per schema; populated as a step in the schema-entry process. Fetched in `fetchSchemaData()` alongside the rest and passed to `buildWeeks()`; deliberately kept out of the `!schemas` throw guard so a missing/empty table falls back rather than bricking the app.

**Content area for non-training days**: When `day.type === "rust"`, either `StretchenCard` (if `day.naam === "Anti-zit"`) or `VrijeDagCard` (all other rust days) is shown; both pull their content from the `rest_day_goals` Supabase table filtered by `dag_van_week` and `g.schema_id === week.schemaId`. Week-specific rows (`week_nummer === week.week`) take priority over catch-all rows (`week_nummer === 0`). When `day.type === "power_hour"`, a hardcoded `PowerHourCard` is shown; when `day.type === "boksen"`, a hardcoded `BoksenCard` is shown — both bypass the exercise sections entirely, including on a day that would otherwise have exercises (a training day overridden to Boksen for one week renders no exercise list, since `buildWeeks` discards the source day's exercises for any non-training effective type). The Progressie chart is shown in all cases (it reads all exercise weights globally). Training sections (Barbell/Spiergroep/KB/Core) are guarded by `{day.type === "training" && ...}`.

**Supabase `schema_days` columns**: `dag_label` TEXT, `dag_volgorde` INTEGER, `dag_nummer` INTEGER (nullable), `type` CHECK in ('training','rust','power_hour','boksen'), `emoji` TEXT, `spiergroep_naam` (→ `naam` in JS). Renamed from `cardio_fitness` (display name "Cardio Fitness") to `power_hour` (display name "Power Hour") — "Cardio Fitness" was the name of the gym, not the training type. `boksen` has no `schema_days` row of its own; it's scheduled per week via `week_overrides` only.
...
The progress note below the sections is driven by `week.phase`: "Opbouw" weeks show a progressive overload tip; "Nieuwe Prikkel" weeks show either an intro tip ("begin with a workable weight, build next week") or a peak tip ("go for max weight"), determined by checking whether `schema.weeks[selectedWeek + 1]?.phase === "Nieuwe Prikkel"` — if true it's the intro week, otherwise it's the peak week. These are two independent `{cond && ...}` blocks with no `else`, so a `"Deload"` week (or any other phase value) shows **no** note — deliberate: the deload facts are already on screen per exercise (`weight_hint` "deload · 60-70%", reduced set counts).
...
A subjective 1–5 score for "hoe ging de training?" and "hoe was je energie voor de training?" (originally "hoe was de motivatie om te trainen?" — question reworded, `score_motivatie` column name unchanged), captured via the same long press that already marks a day complete — the KB block's low tick counts (schema `431ae096`, weeks 31–36) showed that anything adding a new gesture doesn't survive daily use, so this hooks onto the existing one instead.

**Each star row shows a fixed description for whichever value is currently selected** (`TRAINING_SCORE_DESCRIPTIONS`/`ENERGY_SCORE_DESCRIPTIONS`, module scope next to `StarPicker`, keyed 1–5) — added so the scale means the same thing every time it's used, not just whatever a given mood reads into a bare star count. `StarPicker` renders `descriptions[value]` under the row (blank when `value` is 0, i.e. nothing chosen yet), replacing on every tap and pre-filled correctly on reopen since both cases already flow through the same `value` state `ScorePromptModal` already had. A `minHeight` on that line reserves space for the longest entries' two-line wrap so the star row itself never shifts. `ScoreSummary`'s read-only tag next to `StarRow` was renamed from "Motivatie" to "Energie" to match; `score?.motivatie` is still the field read, only the on-screen tag changed.
...
On load, all rows are fetched from the `weights` table and stored in a `weights` map keyed by `"exerciseName__weekNumber"`. Clicking any exercise row expands a small panel with two number inputs labeled **M:** and **Z:** (for each person). On input change, a debounced write fires: non-empty upserts the row, empty **deletes** it (see "Reps logging" below — clearing was a silent client-side-only no-op until 2026-09-16). When the user opens a different exercise, the current exercise's pending change — if any — is flushed to Supabase before switching (`flushSave`). The same flush runs when switching week or day tabs. `flushSave` only acts on a field whose debounce timer is still pending (i.e. actually edited since the last save); merely viewing an already-saved or never-logged exercise and closing it does nothing.
...
person    text       ('M' or 'Z')
weight    numeric
reps      integer    (nullable — logged rep count, see "Reps logging" below)
unique constraint on (exercise, week, person)
```

### Reps logging

Alongside weight, one rep count per exercise per week per person is logged on `weights.reps` (nullable, no CHECK — matches `weight`'s own lack of validation). It rides the exact same write path as weight: `saveMeasurement(exercise, weekNum, person, weightValue, repsValue)` (renamed from `saveWeight`) upserts both columns in one call when `weightValue` is non-empty, and **deletes the row outright** when it's empty (fixed 2026-09-16 — clearing used to silently no-op, only ever clearing local state, never the database; a reload would bring the old weight back). Reps alone never triggers a write or delete — a bodyweight/core exercise, which has no weight field to gate on at all, simply has `reps = NULL` forever, the normal case, not an edge case.

`handleWeightChange` and `flushSave` both mirror into local `weights` state exactly what they send to `saveMeasurement` — for a delete, that means resetting `reps<person>` to `""` too (not falling back to the prescribed number, since the whole row is gone). This was itself a bug (2026-09-16): the reps value sent to the database wasn't always mirrored into local state, so `findPrevWeight` — which reads that state, not the database — could show a stale "no reps" suffix in the same session even though the database was correct; a reload always looked fine, which is why it needed a no-reload verification to catch. `flushSave` additionally only acts on a person whose debounce timer is still pending (not yet fired) — without that gate, closing a panel that was merely viewed, never edited, would re-send an unchanged value or (once empty meant delete) fire a needless delete against a row that never existed.

**Prefill.** `exercises.reps` (a `text` column, previously always `NULL`) was backfilled once via a regex over `exercises.sets` (`^\d+x(\d+)(/\w+)?$`, matching `"3x8"` and `"3x12/been"` alike — the per-limb number is used as-is, since it already means "reps per side"). This covers ~74% of rows; HIIT (`"30s/20s"`), time-hold (`"2x20sec/zij"`), and distance (`"3x20m"`) rows are left `NULL` — genuinely not a rep count. Starting with week 41, `exercises.reps` can also hold a range ("10-12") in parallel with `sets` becoming e.g. "3x10-12" (display-only, unaffected) — convention: the upper bound is the per-session target, the lower bound a floor before increasing weight; the stepper never clamps to either, it's guidance only. `parsePrescribedReps(raw)` (module scope, next to `wKey`) is the single place that parses the raw column: a plain number string parses as before, a range parses to its upper bound, anything absent or unparseable is `null` — never `NaN` (an inline `Number("10-12")` would have produced a literal "NaN" in the stepper with both ± buttons permanently stuck reproducing it, no text input to correct it — this is why it's a real function with a guard, not the bare `Number(e.reps)` it replaced). `buildWeeks`'s `toEx()` carries the result through as `ex.reps` (`Number` or `null`). `prescribedReps`, a plain `{ [wKey]: reps }` lookup built once per render in `FitnessSchema` (after the `schemaLoading` guard — not memoized, same as `CORE_EXERCISES` above it), is consulted by `effectiveReps(exercise, weekNum, person)`: it returns the actually-logged value if one exists, otherwise the prescribed number, otherwise `""`. Everything downstream of `toEx()` (`prescribedReps`, `effectiveReps`, and all three stepper render sites) only ever sees the already-resolved plain integer, regardless of whether it came from a single number or a range — none of them needed to change for range support.

**The prefill is what gets saved, not just displayed.** If the reps stepper is never touched, `effectiveReps`'s fallback value is what `handleWeightChange` bundles into the save the moment a weight is entered — by design, so hitting the prescribed number takes no extra action. Consequence, accepted deliberately: a `weights.reps` value can't be told apart from "the user explicitly confirmed this" versus "this was the prescribed default and nobody touched the stepper." Once ± is tapped once, `weights[wKey].reps<person>` becomes a real value and the fallback stops applying for that row, same "touched vs untouched" signal weight itself already uses.

**The stepper is display-only, never an input** — a `<span>`, not a focusable/tappable element, flanked by ± buttons (floor 1, soft cap 50, empty state renders `–`) — so a mistap can never summon a keyboard. `handleRepsChange(exercise, weekNum, person, value)` updates local state immediately and — only if a weight already exists for that row — schedules the same 500ms-debounced save as `handleWeightChange`, sharing its timer key so whichever of the two fires last wins (always the freshest values of both).

Not in the schema cache (`cached_schema_v5`) and no cache-key bump for either half: logged reps is fetched in `fetchUserData()` alongside `weights` itself (same precedent as `completed_days`/`day_scores`); prescribed reps flows through the schema payload, but a stale cache simply lacking the new `ex.reps` field degrades to "no prefill" — already the correct fallback for HIIT/time/distance rows — not wrong data, so not worth forcing a full cache invalidation over.

Known gap, not fixed: since saving is gated on weight, core and other bodyweight exercises (which have no weight field to begin with — see `ExRow`'s core call site, which passes no weight/expand props at all) have no way to log reps either. For those, rep count is the only progression signal there is; this is unaddressed for now.
...
dag_van_week        text       ('rust' | 'training' | 'power_hour' | 'boksen') — no CHECK constraint on this column
emoji               text       (nullable — tile emoji; defaults: 🏖️ for rust, 🕐 for power_hour, 🥊 for boksen)
naam                text       (nullable — tile label; defaults: 'Vrije dag' for rust, 'Power Hour' for power_hour, 'Boksen' for boksen)
unique constraint on (schema_id, week_nummer, dag_nummer)
```

An override can replace any day — a rest day, Power Hour, or a strength training day — with no restriction in code on which base type it targets; a historical row confirms this in practice (a week-31–36 Power Hour override actually replaces a training day, not a rest day). This is also how Boksen ever appears at all, since it has no `schema_days` row of its own.

Overrides are fetched in `fetchSchemaData()` alongside schemas/schema_days/exercises and passed to `buildWeeks()`. Inside `buildWeeks`, an `overrideMap` keyed by `"calWeek__dagVolgorde"` is built per schema. When a day matches an override: `type` becomes `dag_van_week`; `emoji`/`naam` use the override values (or type-based fallbacks — see `specialActivityDefaults()` for `power_hour`/`boksen`, "Vrije dag"/🏖️ for anything else non-training); `dag_nummer` and `kleur` are set to `null`. `kleur` being null doesn't mean the tile renders gray, though: `WeekDayTile` falls back to a hardcoded `#f97316` for any `power_hour`/`boksen` day (via its own `isSpecialActivity` check) before falling back further to gray — so both render with the orange accent, not neutral gray. The override is baked into `localStorage["cached_schema_v5"]` so it works offline. Pull-to-refresh re-fetches and reapplies overrides.

`power_hour` and `boksen` are grouped under `isSpecialActivity` — not a single shared constant, but the same boolean expression (`day.type === "power_hour" || day.type === "boksen"`) independently declared in two places (once inside `buildWeeks`, once inside `WeekDayTile`), kept in sync by convention rather than by importing a shared value. A third special-activity type would touch four sites, not one: both `isSpecialActivity` declarations, `specialActivityDefaults()` (naam/emoji fallback), and the info-card render gate (`day.type === "power_hour" && <PowerHourCard .../>` / `day.type === "boksen" && <BoksenCard .../>`, which would need a third sibling branch and card component).
...
**Schema caching** — on successful Supabase fetch, the full schema is stored in `localStorage["cached_schema_v5"]` as JSON. On load, the app serves cached data immediately (cache-first), then fetches fresh data in the background and updates the cache. If Supabase is unreachable and a cache exists, a gray "offline" banner appears below the header (`schemaOffline` state). Cache key is `cached_schema_v5` (versioned to avoid stale structure issues).
